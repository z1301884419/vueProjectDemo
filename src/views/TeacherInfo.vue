<template>
  <div class="teInfo">
    ...
    <el-table :data="tableData" style="width: 96%" >
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button class="deleteBtn" type="danger" icon="el-icon-delete" circle></el-button>
    ...
    <!-- 删除老师的模态框 -->
    <!-- <el-dialog title="删除教师" :visible.sync="delTeacher" width="35%">
      
      <span slot="footer" class="dialog-footer">
        <el-button @click="delTeacher = false">取 消</el-button>
        <el-button class="successBtn" @click="delTeacherRequest">确 定</el-button>
      </span>
    </el-dialog> -->
  </div>
</template>
  ...
  methods: {
    ...
    addTeacherRequest(formName){ //添加老师的请求
      ...
      this.$refs[formName].validate((valid) => {
          if (valid) {
            // 这里面写请求
            this.openSuccess("成功", "该教师已成功添加!")
          } else {
            console.log('error submit!!');
            this.$refs[formName].resetFields();
            this.openError("错误", "您的消息填写有误，未能提交!")
            return false;
          }
        });
    },
    modifyDialogShow(row){
      console.log(row);
    }
